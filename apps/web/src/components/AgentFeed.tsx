...
function eventTypeBadge(event: AgentBroadcast): { label: string; tone: string } {
  if (event.type === 'TradeRationale') {
    return {
      label: 'Bet',
      tone: 'border-[var(--cl-accent)]/35 bg-[var(--cl-accent)]/12 text-[var(--cl-accent-soft)]',
    };
  }
  if (event.type === 'NegotiationIntent') {
    return {
      label: 'Intent',
      tone: 'border-[var(--cl-warn)]/35 bg-[var(--cl-warn)]/12 text-[var(--cl-warn-soft)]',
    };
  }
  if (event.type === 'MarketBroadcast') {
    return {
    ...
    <section className="card-lift glass-card rounded-2xl shadow-[0_20px_60px_rgba(0,0,0,0.28)]">
      <div className="border-b border-white/6 px-3.5 py-3 sm:px-4 sm:py-4">
        <div className="flex items-start justify-between gap-3">
          <div>
            <h3 className="text-base font-semibold text-[var(--cl-text-primary)]">Agent Feed</h3>
            <p className="text-sm text-[var(--cl-text-subtle)]">Live stream of agent market moves and conviction.</p>
          </div>
          <div className="text-right text-xs text-[var(--cl-text-dim)]">
          ...
          {FILTERS.map((item) => (
            <button
              ...
              className={`shrink-0 rounded-lg border px-3 py-1.5 text-xs font-medium transition ${
                filter === item.key
                  ? 'border-[var(--cl-accent)]/35 bg-[var(--cl-accent)]/12 text-[var(--cl-accent-soft)]'
                  : 'border-white/8 bg-white/4 text-[var(--cl-text-subtle)] hover:text-[var(--cl-text-primary)]'
              }`}
            >
              {item.label}
            </button>
      ...
      <div className="max-h-[720px] overflow-y-auto p-2.5 sm:p-3">
        {loading ? (
          ...
          <div className="rounded-xl border border-dashed border-white/10 px-4 py-8 text-center text-sm text-[var(--cl-text-dim)]">
          ...
          <div className="space-y-2.5 sm:space-y-3">
            {filtered.slice(0, 120).map((event, index) => {
              ...
              return (
                <article
                  key={event.id}
                  className="animate-card-in card-lift rounded-xl border border-white/6 bg-[var(--cl-surface-2)] p-2.5 sm:p-3"
                ...
                >
                  <div className="mb-1.5 flex flex-wrap items-center justify-between gap-2 sm:mb-2">
                    ...
                    <span className="text-xs text-[var(--cl-text-muted)] sm:text-sm">{relativeTime(event.timestamp)}</span>
                  </div>

                  <div className="text-sm font-semibold text-[var(--cl-text-primary)] sm:text-base">
                    {eventHeadline(event)}
                  </div>
                  ...
                  <p className="reasoning-compact mt-2 text-sm leading-relaxed text-[var(--cl-text-secondary)] sm:mt-3 sm:text-[15px]">
                    {event.reasoning}
                  </p>

                  {showAdvanced && (
                    <div className="mt-2.5 flex flex-wrap gap-1.5 text-xs text-[var(--cl-text-secondary)] sm:mt-3 sm:gap-2 sm:text-sm">
                      {event.sessionId && (
